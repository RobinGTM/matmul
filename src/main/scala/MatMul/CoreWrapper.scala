...
class CoreWrapper(
  ...
  // XPM FIFO ports
  val xpm_ififo_rd = IO(Flipped(new AsyncFIFOReadInterface(UInt(32.W))))
  val xpm_ofifo_wr = IO(Flipped(new AsyncFIFOWriteInterface(UInt(32.W))))
  ...
  withClockAndReset(i_coreclk, ~sync_rstn) {
    ...
    iFifo2AxiS.fifo_rd <> xpm_ififo_rd
    oAxiS2Fifo.fifo_wr <> xpm_ofifo_wr
    // // Input FIFO to AXI-Stream adapter
